...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        nargs="?",
        type=input_path,
        default=working_path,
        help="Path (folder) of images",
    )
    parser.add_argument("-help", action="help", help="show this help message and exit")
    shape_option_group = parser.add_argument_group(
        title="Shape Option",
    ...
    shape_option_group.add_argument(
        "-l", action="store_true", help="Landscape (horizontal)"
    )
    ...
    width_option_group.add_argument(
        "-wg", type=int, help="Width is equal to or greater than WG"
    )
    width_option_group.add_argument(
        "-wl", type=int, help="Width is equal to or less than WL"
    )
    ...
    height_option_group.add_argument(
        "-hg", type=int, help="Height is equal to or greater than HG"
    )
    height_option_group.add_argument(
        "-hl", type=int, help="Height is equal to or less than HL"
    )
    processing_option_group = parser.add_argument_group(
        title="Process for matching image(s)"
    )
    processing_option_group = processing_option_group.add_mutually_exclusive_group()
    processing_option_group.add_argument(
        "-mv", type=os.path.realpath, help="Move matched images to MV"
    )
    processing_option_group.add_argument(
        "-cp", type=os.path.realpath, help="Copy matched images to CP"
    )
    processing_option_group.add_argument(
        "-rm", action="store_true", help="Remove matched images"
    )
    ...
    with os.scandir(args.input_path) as it:
        for entry in it:
            if (
                ...
                and entry.is_file()
                and os.path.splitext(entry.name)[-1] in img_type_list
            ):
